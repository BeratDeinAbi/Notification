...
const DonationModal: React.FC<DonationModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in p-4">
            <div className="bg-surface border border-white/10 rounded-3xl w-full max-w-md relative overflow-hidden shadow-2xl animate-scale-in">
                {/* Decorative Background */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-primary/20 blur-3xl -z-10 rounded-full translate-x-1/2 -translate-y-1/2"></div>
                <div className="absolute bottom-0 left-0 w-64 h-64 bg-pink-500/20 blur-3xl -z-10 rounded-full -translate-x-1/2 translate-y-1/2"></div>
                ...
                <div className="p-6 pb-0 flex justify-between items-start">
                    <div>
                        ...
                        <h2 className="text-2xl font-bold text-white">Projekt unterstützen</h2>
                        <p className="text-slate-400 mt-1 text-sm">Hilf uns, dieses Tool werbefrei und kostenlos zu halten.</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">PayPal</label>
                    ...
                    <div className="relative flex items-center justify-center">
                        ...
                        <span className="relative z-10 bg-surface px-4 text-xs font-medium text-slate-500 uppercase">Oder Krypto</span>
                    </div>

                    {/* Crypto Options */}
                    <div className="space-y-4">
                        {/* Bitcoin */}
                        <div className="bg-white/5 border border-white/5 rounded-xl p-4 transition-colors hover:border-white/10 group">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="text-[10px] bg-white/5 text-slate-400 px-1.5 py-0.5 rounded">BTC Network</span>
                            </div>
                            <div className="flex items-center gap-2 bg-black/20 p-2 rounded-lg">
                                <code className="text-[10px] text-slate-400 truncate flex-1 font-mono">bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh</code>
                                <button
                        ...
                        <div className="bg-white/5 border border-white/5 rounded-xl p-4 transition-colors hover:border-white/10 group">
                            <div className="flex justify-between items-center mb-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-bold text-white text-sm">Ethereum (ETH)</span>
                                </div>
                                <span className="text-[10px] bg-white/5 text-slate-400 px-1.5 py-0.5 rounded">ERC-20</span>
                            </div>
                            <div className="flex items-center gap-2 bg-black/20 p-2 rounded-lg">
                                <code className="text-[10px] text-slate-400 truncate flex-1 font-mono">0x71C7656EC7ab88b098defB751B7401B5f6d8976F</code>
                                <button
                    ...
                    <p className="text-center text-xs text-slate-500">Danke für deinen Support! ❤️</p>
                </div>
            </div>
        </div>
    );
