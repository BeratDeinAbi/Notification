...
const SentimentIndicator: React.FC<SentimentIndicatorProps> = ({ type = 'CRYPTO' }) => {
    const [sentiment, setSentiment] = useState(50); // 0-100 (Bearish-Bullish)
    const [volume, setVolume] = useState(1243);

    useEffect(() => {
        // Reset/Adjust based on type
    ...
    return (
        <div className="bg-surface rounded-2xl border border-subtle p-6">
            <h3 className="text-white font-bold mb-4 flex items-center justify-between">
                ...
                <span className="text-[10px] bg-sky-500/10 text-sky-400 px-2 py-0.5 rounded-full flex items-center gap-1">
            ...
            <div className="mb-4">
                ...
                <div className="h-4 bg-zinc-800 rounded-full overflow-hidden relative">
                    <div
                        className="absolute top-0 bottom-0 left-0 bg-gradient-to-r from-rose-500 via-zinc-500 to-emerald-500 transition-all duration-1000 ease-out"
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
                <div className="bg-zinc-900/50 rounded-lg p-3 border border-zinc-800">
                    ...
                    <div className={`text-xl font-bold font-mono ${sentiment > 55 ? 'text-emerald-400' : sentiment < 45 ? 'text-rose-400' : 'text-yellow-400'}`}>
                        {sentiment.toFixed(0)}/100
                    </div>
                </div>
                <div className="bg-zinc-900/50 rounded-lg p-3 border border-zinc-800">
                    <div className="text-xs text-zinc-500 mb-1">Posts / Stunde</div>
                    <div className="text-xl font-bold text-white font-mono">{volume}</div>
                </div>
            </div>
        </div>
    );
