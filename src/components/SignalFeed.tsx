...
const SignalFeed: React.FC<SignalFeedProps> = ({ signals, onDeleteSignal, onClearAllSignals }) => {
  ...
  return (
    <div className="bg-surface rounded-lg border border-subtle shadow-sm overflow-hidden">
      <div className="p-4 border-b border-subtle bg-zinc-900/50 flex justify-between items-center">
        ...
        {onClearAllSignals && signals.length > 0 && (
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-lg bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 transition-colors flex items-center gap-1.5 border border-rose-500/20"
          >
      ...
      <div className="divide-y divide-subtle max-h-[500px] overflow-y-auto">
        {signals.map((signal) => (
          <div key={signal.id} className="p-4 hover:bg-zinc-800/30 transition-colors animate-signal-enter group">
